...
contract BeastOwnership  is BeastBase, ERC721 {
    ...
    {
        owner = beastIndexToOwner[_tokenId];
    ...
    function tokensOfOwner(address _owner) external view returns(uint256[] ownerTokens) {
        ...
        if (tokenCount == 0) {
            ...
            uint256 totalBeasts = totalSupply();
            ...
            for (beastId = 1; beastId <= totalBeasts; beastId++) {
                if (beastIndexToOwner[beastId] == _owner) {
                    result[resultIndex] = beastId;
                    resultIndex++;
                }
            }
